...
def About():
    ...
    st.write(
        """
    For any inquiries, please contact [kananisahil57@gmail.com].
    """
    )
...
def data_visualization():
    ...
    sns.heatmap(cleaned_df.corr(), annot=True, cmap="coolwarm", linewidths=0.5)
    st.pyplot()
...
def prediction():
    df = pd.read_csv("house_price_prediction_dataset.csv")
    df = df.drop(
        columns=["proximity_to_city_center", "neighborhood_quality", "lot_size"]
    )
    ...
    num_bedrooms = st.number_input(
        "Enter the number of bedrooms:", min_value=1, max_value=10, value=3
    )
    num_bathrooms = st.number_input(
        "Enter the number of bathrooms:", min_value=1, max_value=5, value=2
    )
    square_footage = st.number_input(
        "Enter the square footage:", min_value=300, max_value=10000, value=1500
    )
    age_of_house = st.number_input(
        "Enter the age of the house:", min_value=0, max_value=150, value=20
    )
